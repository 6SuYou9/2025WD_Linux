...
int main(int argc,char *argv[])
{
    ...
    event.events = EPOLLIN;     // 说明这是读就绪,EPOLLET edge-trigger 说明netfd边缘触发
    event.data.fd = STDIN_FILENO;   // 将来STDIN就绪之后，把文件描述符填入队列
    // 设置stdin的监听
    epoll_ctl(epfd,EPOLL_CTL_ADD,STDIN_FILENO,&event);
    // epoll_ctl 中的event是一个传入传出参数，内容可能会被改掉
    event.events = EPOLLIN|EPOLLET; // 说明这是读就绪,EPOLLET edge-trigger 说明netfd边缘触发
